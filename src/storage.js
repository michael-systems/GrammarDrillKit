...
function createDefaultData() {
  return { version: SCHEMA_VERSION, bestResults: {}, mistakes: {}, lastPracticed: {}, theme: null };
}
...
function sanitizeBestResults(value) {
  ...
  for (const [moduleId, levels] of Object.entries(value)) {
    ...
    for (const [level, result] of Object.entries(levels)) {
      ...
      if (Number.isInteger(percentage) && percentage >= 0 && percentage <= 100 && Number.isInteger(correct) && correct >= 0 && Number.isInteger(total) && total > 0 && correct <= total && completedAt) {
        cleanLevels[level] = { percentage, correct, total, completedAt };
      }
    }
    if (Object.keys(cleanLevels).length > 0) bestResults[moduleId] = cleanLevels;
  }
  return bestResults;
}
...
function sanitizeMistakes(value) {
  ...
  for (const [questionId, mistake] of Object.entries(value)) {
    ...
    if (moduleId && level && Number.isInteger(correctReviewCount) && correctReviewCount >= 0 && correctReviewCount <= 1) {
      mistakes[questionId] = { moduleId, level, correctReviewCount, addedAt };
    }
  }
  return mistakes;
}
...
function sanitizeLastPracticed(value) {
  if (!isPlainObject(value)) return {};
  return Object.fromEntries(Object.entries(value).filter(([, date]) => typeof date === 'string'));
}

export function sanitizeData(value) {
  if (!isPlainObject(value) || value.version !== SCHEMA_VERSION) return createDefaultData();
  return {
...
export function createProgressStore(adapter) {
  ...
  function persist() {
    try { adapter?.setItem?.(STORAGE_KEY, JSON.stringify(data)); } catch { /* storage must never block the app */ }
  }
  ...
  function getData() { return structuredClone(data); }
  function setData(nextData) { data = sanitizeData(nextData); persist(); return getData(); }

  load();

  return {
    getData,
    setTheme(theme) { data.theme = sanitizeTheme(theme); persist(); },
    recordBestResult(moduleId, level, score, completedAt = new Date().toISOString()) {
      const next = { percentage: score.percentage, correct: score.correct, total: score.total, completedAt };
      data.bestResults[moduleId] ??= {};
      const current = data.bestResults[moduleId][level];
      if (!current || next.percentage > current.percentage || (next.percentage === current.percentage && next.total > current.total)) {
    ...
    setLastPracticed(moduleId, date = new Date().toISOString().slice(0, 10)) { data.lastPracticed[moduleId] = date; persist(); },
    addMistake(question, addedAt = new Date().toISOString()) {
      data.mistakes[question.id] = { moduleId: question.moduleId, level: question.level, correctReviewCount: 0, addedAt };
      persist();
    },
    ...
    cleanupStaleMistakes(validQuestionIds) {
      ...
      for (const id of Object.keys(data.mistakes)) {
        if (!validQuestionIds.has(id)) { delete data.mistakes[id]; changed = true; }
      }
      if (changed) persist();
      return changed;
    },
